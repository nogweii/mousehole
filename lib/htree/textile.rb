...
module HTree
  ...
  module Node
    def display_textile(out=$stdout, encoding=HTree::Encoder.internal_charset)
      encoder = HTree::Encoder.new(encoding)
      self.to_textile(encoder, HTree::DefaultContext)
  ...
  class Name
    def to_textile(out, context)
      ...
      elsif HTree::TEXTILE_QTAGS[qualified_name]
        out.output_string HTree::TEXTILE_QTAGS[qualified_name]
      else
  ...
  class STag
    ...
    def to_textile_stag(out, context)
      if HTree::TEXTILE_QTAGS[@name.qualified_name]
